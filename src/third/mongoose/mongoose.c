...
static void ff_list(const char *dir, void (*fn)(const char *, void *),
  ...
  DIR d;
  FILINFO fi;
  if (f_mg_opendir(&d, dir) == FR_OK) {
    while (f_mg_readdir(&d, &fi) == FR_OK && fi.fname[0] != '\0') {
...
struct dirent *mg_readdir(DIR *d) {
...
static void p_list(const char *dir, void (*fn)(const char *, void *),
  ...
  struct dirent *dp;
  DIR *dirp;
  if ((dirp = (mg_opendir(dir))) == NULL) return;
