...
int InitPlatform(void)
{
    ...
    TRACELOG(LOG_INFO, "DISPLAY: opening default gracphic card");
    TRACELOG(LOG_INFO, DEFAULT_GRAPHIC_DEVICE_DRM);
    ...
    TRACELOG(LOG_INFO, "DISPLAY: Connectors found: %i", res->count_connectors);

    for (size_t i = 0; i < res->count_connectors; i++)
    {
        TRACELOG(LOG_INFO, "DISPLAY: Connector index %i", i);

        drmModeConnector *con = drmModeGetConnector(platform.fd, res->connectors[i]);
        TRACELOG(LOG_INFO, "DISPLAY: Connector modes detected: %i", con->count_modes);

        if ((con->connection == DRM_MODE_CONNECTED) && (con->encoder_id))
        {
            TRACELOG(LOG_INFO, "DISPLAY: DRM mode connected");
            platform.connector = con;
            break;
        }
        else
        {
            TRACELOG(LOG_INFO, "DISPLAY: DRM mode NOT connected (deleting)");
    ...
    {
        TRACELOG(LOG_INFO, "DISPLAY: Selecting DRM connector mode for current used mode...");
    ...
    if (platform.device == EGL_NO_DISPLAY) {
        PFNEGLGETPLATFORMDISPLAYEXTPROC eglGetPlatfromDisplay = (PFNEGLGETPLATFORMDISPLAYEXTPROC)eglGetProcAddress("eglGetPlatformDisplayEXT");
        platform.device = eglGetPlatfromDisplay(EGL_PLATFORM_GBM_MESA, platform.gbmDevice, NULL);
    }
    if (platform.device == EGL_NO_DISPLAY)
    {
    ...
    TRACELOG(LOG_INFO, "DISPLAY: EGL configs available: %d", numConfigs);
    ...
    TRACELOG(LOG_INFO, "DISPLAY: EGL matching configs available: %d", matchingNumConfigs);
    ...
    for (EGLint i = 0; i < matchingNumConfigs; ++i)
    {
        ...
        {
            TRACELOG(LOG_INFO, "DISPLAY: Using EGL config: %d", i);
    ...
    platform.surface = eglCreatePlatformWindowSurfaceEXT(platform.device, platform.config, (EGLNativeWindowType)platform.gbmSurface, NULL);
...
static int FindMatchingConnectorMode(const drmModeConnector *connector, const drmModeModeInfo *mode)
{
    ...
    {
        TRACELOG(LOG_INFO, "DISPLAY: DRM mode: %d %ux%u@%u %s", i, connector->modes[i].hdisplay, connector->modes[i].vdisplay,
...
{
    TRACELOG(LOG_INFO, "DISPLAY: Searching exact connector mode for %ux%u@%u, selecting an interlaced mode is allowed: %s", width, height, fps, allowInterlaced? "yes" : "no");
    ...
    for (int i = 0; i < platform.connector->count_modes; i++)
    {
        ...
        TRACELOG(LOG_INFO, "DISPLAY: DRM Mode %d %ux%u@%u %s", i, mode->hdisplay, mode->vdisplay, mode->vrefresh, (mode->flags & DRM_MODE_FLAG_INTERLACE)? "interlaced" : "progressive");
    ...
    TRACELOG(LOG_INFO, "DISPLAY: No DRM exact matching mode found");
    return -1;
}
...
{
    TRACELOG(LOG_INFO, "DISPLAY: Searching nearest connector mode for %ux%u@%u, selecting an interlaced mode is allowed: %s", width, height, fps, allowInterlaced? "yes" : "no");
    ...
    for (int i = 0; i < platform.connector->count_modes; i++)
    {
        ...
        TRACELOG(LOG_INFO, "DISPLAY: DRM mode: %d %ux%u@%u %s", i, mode->hdisplay, mode->vdisplay, mode->vrefresh,
            (mode->flags & DRM_MODE_FLAG_INTERLACE)? "interlaced" : "progressive");

        if ((mode->hdisplay < width) || (mode->vdisplay < height))
        {
            TRACELOG(LOG_INFO, "DISPLAY: DRM mode is too small");
            continue;
        }

        if ((mode->flags & DRM_MODE_FLAG_INTERLACE) && (!allowInterlaced))
        {
            TRACELOG(LOG_INFO, "DISPLAY: DRM shouldn't choose an interlaced mode");
            continue;
        }
